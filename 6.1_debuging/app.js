...
const food = (arr) => {
    ...
    arr.forEach((el) => {
        el.favoriteFoods.forEach((foodObj) => {
            //console.log(foodObj);
            const arrFromObj = Array.from(foodObj.meats).concat(Array.from(foodObj.fish));//foodObj it is object and we need to transfere him to the array
            arrFromObj.forEach((meatItem) => { 
